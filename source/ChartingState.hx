			...
			else
			{
					...
					&& FlxG.mouse.y < gridBG.y + (GRID_SIZE * _song.notes[curSection].lengthInSteps))
				{
					...
					if (FlxG.mouse.justPressedRight)
						addNote(specialNote = true);
					else
						addNote(specialNote = false);
				}
			}
		}
...
// || FlxG.keys.justPressed.X  || FlxG.keys.justPressed.C || FlxG.keys.justPressed.V
		...
		for (i in section)
			{
				...
				var note:Note = new Note(daStrumTime, daNoteInfo % 4, true, null, false, i[3]);
	...
	private function addNote(specialNote:Bool):Void
	{
