...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		stageSongs = ["bopeebo", "fresh", "dadbattle"];
		...
		else if (campSongs.contains(SONG.song.toLowerCase()))
		{
			defaultCamZoom = 0.65;
			curStage = 'camp';
			var sky:FlxSprite = new FlxSprite(-600, -300).loadGraphic('assets/images/flippy/sky.png');
			sky.antialiasing = true;
			sky.scrollFactor.set(0.5, 0.5);
			sky.active = false;
			add(sky);

			var backTrees:FlxSprite = new FlxSprite(-625, -325).loadGraphic('assets/images/flippy/backtrees.png');
			...
			var trees:FlxSprite = new FlxSprite(-625, -275).loadGraphic('assets/images/flippy/trees.png');
			trees.updateHitbox();
			...
			var ground:FlxSprite = new FlxSprite(-700, -150).loadGraphic('assets/images/flippy/ground.png');
			ground.updateHitbox();
			ground.antialiasing = true;
			ground.active = false;
			add(ground);

			table = new FlxSprite(-700, 25).loadGraphic('assets/images/flippy/table.png');
		...
		var camPos:FlxPoint = new FlxPoint(dad.getGraphicMidpoint().x += 200, dad.getGraphicMidpoint().y += 200);
	...
	{
		var black:FlxSprite = new FlxSprite(-400, -200).makeGraphic(FlxG.width * 2, FlxG.height * 2, FlxColor.BLACK);
					...
					if (daNote.alpha > 0.3){
						...
						if(Config.newInput){
							if (daNote.camoNote == true)
								{
									health -= 0.4 * Config.healthDrainMultiplier;
									misses += 1;
									updateAccuracy();
									vocals.volume = 0;
								}
							else if (daNote.nadeNote == true)
								{
									trace("Good Miss!");
								}
							else
								{
									health -= 0.055 * Config.healthDrainMultiplier;
									misses += 1;
									updateAccuracy();
									vocals.volume = 0;
								}
